...
var Ed3d = {
    ...
    launchMap: function() {
        ...
        Ed3d.initScene();
        Ed3d.grid1H = $.extend({}, Grid.init(100, 1121827, 0), {});
        Ed3d.grid1K = $.extend({}, Grid.init(1000, 2241082, 1000), {});
        Ed3d.grid1XL = $.extend({}, Grid.infos(10000, 2241082, 10000), {});
        Ed3d.skyboxStars();
        HUD.create("ed3dmap");
    ...
    initScene: function() {
        ...
        light = new THREE.HemisphereLight(16777215,13421772);
        scene.add(light);
        ...
        scene.fog = new THREE.FogExp2(855312,0.000128);
    ...
    skyboxStars: function() {
        ...
        for (var p = 0; p < 5; p++) {
            var particle = new THREE.Vector3(Math.random() * sizeStars - (sizeStars / 2),Math.random() * sizeStars - (sizeStars / 2),Math.random() * sizeStars - (sizeStars / 2));
...
function animate(time) {
    ...
    Ed3d.starfield.position.set(controls.target.x - (controls.target.x / 10) % 4000, controls.target.y - (controls.target.y / 10) % 4000, controls.target.z - (controls.target.z / 10) % 4000);
...
function enableFarView(scale, withAnim) {
    if (isFarView || this.Galaxy == null) {
        return
    }
    if (withAnim == undefined) {
        withAnim = true
    }
    isFarView = true;
    var scaleFrom = {
    ...
    if (withAnim) {
        var obj = this;
        Ed3d.tween = new TWEEN.Tween(scaleFrom,{
            override: true
        }).to(scaleTo, 500).start().onUpdate(function() {
...
function refreshWithCamPos() {
    ...
    if (camSave.x == Math.round(camera.position.x / p) * p && camSave.y == Math.round(camera.position.y / p) * p && camSave.z == Math.round(camera.position.z / p) * p) {
        return
    }
...
var Loader = {
    start: function() {
        $("#loader").remove();
        $("<div></div>").attr("id", "loader").html(Loader.svgAnim).css("color", "rgb(200, 110, 37)").css("font-size", "1.5rem").css("font-family", "Helvetica").css("font-variant", "small-caps").appendTo("#ed3dmap");
...
var Action = {
    ...
    init: function() {
        ...
        container.addEventListener("mousedown", function(e) {
            obj.onMouseDown(e, obj)
        }, false);
        container.addEventListener("mouseup", function(e) {
            obj.onMouseUp(e, obj)
        }, false);
        container.addEventListener("mousemove", function(e) {
            obj.onMouseHover(e, obj)
        }, false);
        container.addEventListener("mousewheel", this.stopWinScroll, false);
        container.addEventListener("DOMMouseScroll", this.stopWinScroll, false);
    ...
    sizeOnScroll: function(scale) {
        if (System.particle == undefined || scale <= 0) {
            return
        }
    ...
    onMouseHover: function(e, obj) {
        ...
        obj.mouseVector = new THREE.Vector3(((e.clientX - position.left) / renderer.domElement.width) * 2 - 1,-((e.clientY - position.top) / renderer.domElement.height) * 2 + 1,1);
        ...
        if (intersects.length > 0) {
            for (var i = 0; i < intersects.length; i++) {
                ...
                if (intersection.object.clickable) {
                    ...
                    var selPoint = intersection.object.geometry.vertices[indexPoint];
                    if (selPoint.visible) {
                        $("#hud #infos").html("<h2>" + selPoint.name + "</h2>");
    ...
    moveNextPrev: function(indexPoint, increment) {
        ...
        while (!find) {
            if (indexPoint < 0) {
                indexPoint = System.particleGeo.vertices.length - 1
            } else {
                if (System.particleGeo.vertices[indexPoint] == undefined) {
                    indexPoint = 0
                }
            }
    ...
    moveInitalPosition: function(timer) {
        ...
        Ed3d.tween = new TWEEN.Tween(moveFrom,{
            override: true
        }).to(moveCoords, timer).start().onUpdate(function() {
    ...
    moveToObj: function(index, obj) {
        if (this.oldSel !== null && this.oldSel == index) {
            return false
        }
        controls.enabled = false;
        HUD.setInfoPanel(index, obj);
        if (obj.infos != undefined) {
            HUD.openHudDetails()
        }
        this.oldSel = index;
        var goX = obj.x;
        ...
        Ed3d.tween = new TWEEN.Tween(moveFrom,{
            override: true
        }).to(moveCoords, 800).start().onUpdate(function() {
            camera.position.set(moveFrom.x, moveFrom.y, moveFrom.z);
            controls.target.set(moveFrom.mx, moveFrom.my, moveFrom.mz)
        }).onComplete(function() {
            controls.update()
...
var Galaxy = {
    ...
    infosShow: function() {
        ...
        if (this.infos !== null) {
            this.infos.visible = Ed3d.showGalaxyInfos
        }
    },
    infosHide: function() {
        if (this.infos !== null) {
    ...
    infosUpdateCallback: function(scale) {
        if (!Ed3d.showGalaxyInfos || this.infos == null) {
            return
        }
        scale -= 70;
        ...
        if (this.infos.previousOpacity == opacity) {
            return
        }
        ...
        for (var i = 0; i < pix.length; i += 20) {
            ...
            if (avg > min) {
                ...
                for (var y = -density; y < density; y = y + add) {
                    var particle = new THREE.Vector3(x + ((Math.random() - 0.5) * 25),(y * 10) + ((Math.random() - 0.5) * 50),z + ((Math.random() - 0.5) * 25));
                    ...
                    if (density >= 2 && Math.abs(y) - 1 == 0 && Math.random() * 1000 < 200) {
                        ...
                        if (density < 4 || (Math.random() * 1000 < 400 - (density * 2))) {
                            particles.vertices.push(particle);
...
var Grid = {
    ...
    addCoords: function() {
        ...
        if (this.coordGrid != null) {
            if (Math.abs(camera.position.y - this.obj.position.y) > this.size * 10 || Math.abs(camera.position.y - this.obj.position.y) < this.obj.minDistView) {
...
var HUD = {
    ...
    create: function(container) {
        ...
        if (!$("#" + this.container + " #controls").length && Ed3d.withOptionsPanel == true) {
            $("#" + this.container).append('  <div id="controls">    <a data-view="3d" class="view selected">3D</a>    <a data-view="top" class="view">2D</a>    <a data-view="infos" class="' + (Ed3d.showGalaxyInfos ? "selected" : "") + '">i</a>    <a data-view="options">' + Ico.cog + '</a>    <div id="options" style="display:none;"></div>  </div>');
            this.createSubOptions();
            if (Ed3d.withFullscreenToggle) {
                $("<a></a>").attr("id", "tog-fullscreen").html("Fullscreen").click(function() {
                    $("#" + container).toggleClass("map-fullscreen");
                    refresh3dMapSize()
                }).prependTo("#controls")
            }
        }
        if (!Ed3d.withHudPanel) {
            return
        }
        $("#" + this.container).append('<div id="hud"></div>');
        $("#hud").append('<div>    <h2>Infos</h2>     Dist. Sol <span id="distsol"></span>    <div id="coords" class="coords">      <span id="cx"></span><span id="cy"></span><span id="cz"></span></div>      <p id="infos"></p>    </div>  <div id="filters">  </div></div>');
    ...
    createSubOptions: function() {
        $("<a></a>").addClass("sub-opt active").html("Toggle Milky Way").click(function() {
        ...
        $("<a></a>").addClass("sub-opt active").html("Toggle grid").click(function() {
    ...
    initControls: function() {
        $("#controls a").click(function(e) {
            ...
            case "3d":
                ...
                var moveCoords = {
                    x: controls.target.x - 100,
                    y: controls.target.y + 500,
                    z: controls.target.z + 500
                };
                HUD.moveCamera(moveFrom, moveCoords);
                break;
            case "infos":
    ...
    moveCamera: function(from, to) {
        Ed3d.tween = new TWEEN.Tween(from,{
            override: true
        }).to(to, 800).start().onUpdate(function() {
    ...
    initHudAction: function() {
        ...
        $(".map_filter").click(function(e) {
            ...
            $(Ed3d.catObjs[idCat]).each(function(key, indexPoint) {
                ...
                System.particleGeo.colors[indexPoint] = (active == 1) ? obj.color : new THREE.Color("#111111");
                ...
                if (center == null) {
                    ...
                    center.set((center.x + obj.x), (center.y + obj.y), (center.z + obj.z));
                    if ((Math.abs(pointFar.x) - Math.abs(obj.x)) + (Math.abs(pointFar.y) - Math.abs(obj.y)) + (Math.abs(pointFar.z) - Math.abs(obj.z)) < 0) {
            ...
            center.set(Math.round(center.x / nbPoint), Math.round(center.y / nbPoint), -Math.round(center.z / nbPoint));
            $(this).data("active", active);
            $(this).toggleClass("disabled");
            if (Action.oldSel != null && !Action.oldSel.visible) {
                Action.disableSelection()
            }
            var distance = pointFar.distanceTo(center) + 200;
            Ed3d.playerPos = [center.x, center.y, center.z];
            Ed3d.cameraPos = [center.x + (Math.floor((Math.random() * 100) + 1) - 50), center.y + distance, center.z - distance];
    ...
    addFilter: function(groupId, idCat, val) {
        ...
        $("#" + groupId).append('<a class="map_filter" data-active="1" data-filter="' + idCat + '"><span class="check" style="background:' + back + '"> </span>' + val.name + "</a>")
    },
    openHudDetails: function() {
        $("#hud").hide();
        $("#systemDetails").show().hover(function() {
            controls.enabled = false
        }, function() {
            controls.enabled = true
        })
    ...
    setInfoPanel: function(index, point) {
        $("#systemDetails").html("<h2>" + point.name + '</h2><div class="coords">  <span>' + point.x + "</span><span>" + point.y + "</span><span>" + (-point.z) + '</span></div>  <p id="infos"></p></div>' + (point.infos != undefined ? "<div>" + point.infos + "</div>" : "") + '<div id="nav"></div>');
        $("<a/>", {
            html: "<"
        }).click(function() {
            Action.moveNextPrev(index - 1, -1)
        }).appendTo("#nav");
        $("<a/>", {
            html: "X"
        }).click(function() {
    ...
    createRoute: function(idRoute, route) {
        ...
        $.each(route.points, function(key2, val) {
            if (Route.systems[val.s] !== false) {
                ...
                last = [c[0], c[1], c[2]];
                geometryL.vertices.push(new THREE.Vector3(c[0],c[1],c[2]))
            } else {
        ...
        if (first !== null) {
            this.addCircle("route-" + idRoute + "-first", first, color, 7)
        }
        if (!hideLast && last !== null) {
            this.addCircle("route-" + idRoute + "-last", last, color, 3)
        }
            ...
            for (i = 0; i < length; ) {
                ...
                case "q":
                    ...
                    if (laste) {
                        ...
                        cpy0 = laste.y;
                        for (i2 = 1,
                        divisions = this.divisions; i2 <= divisions; i2++) {
                            var t = i2 / divisions;
                            b2(t, cpx0, cpx1, cpx);
                            b2(t, cpy0, cpy1, cpy)
                        }
